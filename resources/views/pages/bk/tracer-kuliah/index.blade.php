@extends('layouts.app', ['title' => 'Tracer Study Kuliah'])

@section('content')
    <x-common.page-breadcrumb pageTitle="Tracer Study Kuliah" />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-xs">
        ...
        <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-gray-800">
            <!-- Search & Filters -->
            <form action="{{ route('bk.tracer-kuliah.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3 w-full sm:max-w-2xl">
                <!-- Search input -->
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama alumni atau kampus..."
                ...
                <div class="w-full sm:w-40">
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="putus" {{ $status === 'putus' ? 'selected' : '' }}>Putus Kuliah</option>
                    </select>
                </div>
            </form>

            <!-- Add Button -->
            <a href="{{ route('bk.tracer-kuliah.create') }}"
                    ...
                    @forelse ($tracers as $row)
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap font-semibold text-gray-800 dark:text-white/95">
                                {{ $row->alumniKuliah->nama_alumni ?? $row->nama_alumni ?? '-' }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap font-medium text-xs">
                                ...
                                @elseif($row->status_kuliah === 'lulus')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-600 dark:text-gray-400">
                                {{ $row->tanggal_update ? $row->tanggal_update->format('d M Y') : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium">
                                <a href="{{ route('bk.tracer-kuliah.edit', $row->id) }}" class="text-yellow-600 hover:text-yellow-900 dark:text-yellow-400 dark:hover:text-yellow-300">
                                    Ubah
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-gray-400 dark:text-gray-500">
                                Tidak ada data Tracer Study Kuliah ditemukan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
document.getElementById('bulkDeleteBtn').addEventListener('click', function() {
    ...
    Swal.fire({
        ...
        text: `Anda akan menghapus ${ids.length} data Tracer Kuliah. Tindakan ini tidak dapat dibatalkan!`,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            fetch('{{ route("bk.tracer-kuliah.bulk-delete") }}', {
                method: 'POST',
                headers: {
