@extends('layouts.app', ['title' => 'Ubah Tracer Study Kuliah'])

@section('content')
    <x-common.page-breadcrumb pageTitle="Ubah Tracer Study Kuliah" />

    {{-- GANTI INI: Tambah mx-auto untuk center --}}
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-xs max-w-2xl mx-auto">
        <div class="p-6 border-b border-gray-100 dark:border-gray-800">
            <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">
                Formulir Ubah Data Tracer Study Kuliah
            </h3>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('bk.tracer-kuliah.update', $tracer->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="alumni_kuliah_id" name="alumni_kuliah_id" required
                        ...
                        @foreach($alumniList as $alumni)
                            <option value="{{ $alumni->id }}" {{ old('alumni_kuliah_id', $tracer->alumni_kuliah_id) == $alumni->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" id="kampus_tujuan" name="kampus_tujuan" value="{{ old('kampus_tujuan', $tracer->kampus_tujuan) }}"
                ...
                <div>
                    ...
                    <input type="text" id="program_studi" name="program_studi" value="{{ old('program_studi', $tracer->program_studi) }}"
                ...
                <div>
                    ...
                    <select id="status_kuliah" name="status_kuliah" required
                        ...
                        <option value="aktif" {{ old('status_kuliah', $tracer->status_kuliah) === 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="lulus" {{ old('status_kuliah', $tracer->status_kuliah) === 'lulus' ? 'selected' : '' }}>Lulus</option>
                        <option value="cuti" {{ old('status_kuliah', $tracer->status_kuliah) === 'cuti' ? 'selected' : '' }}>Cuti</option>
                        <option value="putus" {{ old('status_kuliah', $tracer->status_kuliah) === 'putus' ? 'selected' : '' }}>Putus Kuliah</option>
                    </select>
                    @error('status_kuliah')
                        <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea id="detail_status" name="detail_status" rows="3" placeholder="Keterangan tambahan tentang status kuliah..."
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('detail_status', $tracer->detail_status) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="testimoni" name="testimoni" rows="4" placeholder="Kesan selama bersekolah atau saran pengembangan sekolah..."
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('testimoni', $tracer->testimoni) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="tanggal_update" name="tanggal_update" value="{{ old('tanggal_update', $tracer->tanggal_update?->format('Y-m-d') ?? date('Y-m-d')) }}"
