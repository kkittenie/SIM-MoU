@extends('layouts.app', ['title' => 'Tambah Alumni Bekerja'])

@section('content')
    <x-common.page-breadcrumb pageTitle="Tambah Alumni Bekerja" />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-xs max-w-2xl">
        <div class="p-6 border-b border-gray-100 dark:border-gray-800">
            <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">
                Formulir Tambah Alumni Bekerja Baru
            </h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Isi data di bawah ini untuk mendata alumni yang sudah bekerja.
            </p>
        </div>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('bkk.alumni-bekerja.store') }}" method="POST" class="space-y-6" x-data="{ sumber: 'mitra' }">
                @csrf
                ...
                <div x-show="sumber === 'mitra'" x-transition>
                    ...
                    <select id="perusahaan_mitra_id" name="perusahaan_mitra_id" :required="sumber === 'mitra'"
                        ...
                        @foreach($mitras as $mitra)
                            <option value="{{ $mitra->id }}" {{ old('perusahaan_mitra_id') == $mitra->id ? 'selected' : '' }}>{{ $mitra->nama_perusahaan }} ({{ $mitra->bidang_industri }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div x-show="sumber === 'manual'" x-transition class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="bidang_industri" name="bidang_industri" value="{{ old('bidang_industri') }}" :required="sumber === 'manual'" placeholder="e.g. Teknologi / IT, Manufaktur, Jasa"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="jabatan" name="jabatan" value="{{ old('jabatan') }}" placeholder="e.g. Junior Web Developer" required
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="gaji" name="gaji" value="{{ old('gaji') }}" placeholder="e.g. 5000000"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="status_pekerjaan" name="status_pekerjaan" required
                            ...
                            <option value="Kontrak" {{ old('status_pekerjaan') === 'Kontrak' ? 'selected' : '' }}>Kontrak</option>
                            <option value="Magang" {{ old('status_pekerjaan') === 'Magang' ? 'selected' : '' }}>Magang / Apprentice</option>
                    ...
                    <div>
                        ...
                        <select id="lokasi_kerja" name="lokasi_kerja" required
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                            <option value="Dalam Negeri" {{ old('lokasi_kerja', 'Dalam Negeri') === 'Dalam Negeri' ? 'selected' : '' }}>Dalam Negeri</option>
                            <option value="Luar Negeri" {{ old('lokasi_kerja') === 'Luar Negeri' ? 'selected' : '' }}>Luar Negeri</option>
                        </select>
                    </div>
                </div>
