@extends('layouts.app', ['title' => 'Tambah Lowongan Kerja'])

@section('content')
    <x-common.page-breadcrumb pageTitle="Tambah Lowongan Kerja" />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-xs max-w-2xl">
        <div class="p-6 border-b border-gray-100 dark:border-gray-800">
            <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">
                Formulir Tambah Lowongan Kerja Baru
            </h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Lengkapi formulir di bawah ini untuk mempublikasikan lowongan pekerjaan baru.
            </p>
        </div>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('bkk.lowongan-kerja.store') }}" method="POST" class="space-y-6" x-data="{ sumber: 'mitra' }">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" id="judul" name="judul" value="{{ old('judul') }}" placeholder="e.g. Lowongan Web Developer Senior" required
                ...
                <!-- Posisi -->
                <div>
                    <label sku for="posisi" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                        Posisi Kerja <span class="text-error-500">*</span>
                    </label>
                    <input type="text" id="posisi" name="posisi" value="{{ old('posisi') }}" placeholder="e.g. Backend Engineer" required
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                </div>
                ...
                <div x-show="sumber === 'mitra'" x-transition>
                    ...
                    <select id="perusahaan_mitra_id" name="perusahaan_mitra_id" :required="sumber === 'mitra'"
                        ...
                        @foreach($mitras as $mitra)
                            <option value="{{ $mitra->id }}" {{ old('perusahaan_mitra_id') == $mitra->id ? 'selected' : '' }}>{{ $mitra->nama_perusahaan }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div x-show="sumber === 'manual'" x-transition>
                    ...
                    <input type="text" id="perusahaan_nama" name="perusahaan_nama" value="{{ old('perusahaan_nama') }}" :required="sumber === 'manual'" placeholder="e.g. PT Ruangguru Indonesia"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                </div>
                ...
                <div>
                    ...
                    <textarea id="persyaratan" name="persyaratan" rows="4" placeholder="Masukkan persyaratan lowongan (gunakan enter atau tanda hubung untuk daftar baru)" required
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('persyaratan') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" placeholder="Masukkan deskripsi tanggung jawab atau info tambahan lowongan kerja"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="gaji" name="gaji" value="{{ old('gaji') }}" placeholder="e.g. Rp 4.500.000 - Rp 6.000.000"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                    </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                        <option value="Aktif" {{ old('status', 'Aktif') === 'Aktif' ? 'selected' : '' }}>Aktif (Tampilkan)</option>
                        <option value="Tutup" {{ old('status') === 'Tutup' ? 'selected' : '' }}>Tutup (Sembunyikan)</option>
                    </select>
                </div>
