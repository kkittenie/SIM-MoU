...
@section('content')
    <x-common.page-breadcrumb pageTitle="Kelola Pengguna" />
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-xs">
        ...
        <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-gray-800">
            <!-- Search Form -->
            <form action="{{ route('pengguna.index') }}" method="GET" class="w-full sm:max-w-xs">
                <div class="relative">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-transparent">
                    @forelse ($users as $user)
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-800 dark:text-white/95">
                                {{ $user->name }}
                                @if($user->id === auth()->id())
                                    <span class="ml-1 text-xs text-gray-400 font-normal italic">(Anda)</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $user->email }}</td>
                            <td class="px-6 py-4 whitespace-nowrap capitalize">
                                <span class="inline-flex items-center rounded-md px-2.5 py-0.5 text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200">
                                    {{ $user->role }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('pengguna.show', $user->id) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300">
                                        Detail
                                    </a>
                                    <span class="text-gray-300 dark:text-gray-700">|</span>
                                    <a href="{{ route('pengguna.edit', $user->id) }}" class="text-yellow-600 hover:text-yellow-900 dark:text-yellow-400 dark:hover:text-yellow-300">
                                        Ubah
                                    </a>
                                    @if ($user->id !== auth()->id())
                                        <span class="text-gray-300 dark:text-gray-700">|</span>
                                        <form action="{{ route('pengguna.destroy', $user->id) }}" method="POST" class="inline-block delete-form" 
                                            data-name="{{ $user->name }}">
                                            @csrf
                                            @method('DELETE')
